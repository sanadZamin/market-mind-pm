...
REMOTE_SCRIPT="${env.WORKSPACE}/deploy/jenkins-remote-deploy.sh"
if [ ! -f "\$REMOTE_SCRIPT" ]; then
  echo "ERROR: missing \$REMOTE_SCRIPT (is Checkout stage enabled?)"
  exit 1
fi
...
remote_rc=\$?
set -e
echo "Remote deploy exit code: \$remote_rc"
exit \$remote_rc
"""
                    }
